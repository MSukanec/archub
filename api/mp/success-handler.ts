...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    // Redirigir a la página unificada según status
    if (status === 'approved') {
      return res.redirect(302, `/checkout/payment-return?course_slug=${finalCourseSlug}&provider=mercadopago`);
    } else if (status === 'pending') {
      return res.redirect(302, `/learning/courses/${finalCourseSlug}?payment=pending`);
    } else {
      return res.redirect(302, `/learning/courses/${finalCourseSlug}?payment=failed`);
    }
