...
export async function getDashboardStats(
  ...
  try {
    ...
    const [
      ...
      // Total courses
      ctx.supabase.from('courses').select('id', { count: 'exact', head: true }).eq('is_deleted', false),
      
      // Active courses
      ctx.supabase.from('courses').select('id', { count: 'exact', head: true }).eq('is_deleted', false).eq('is_active', true),
