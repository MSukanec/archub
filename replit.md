...
- **Page Architecture (3-Layer Pattern)**: Clean separation between **Page** (orquestación + layout selection + tab state), **Layout** (estructura: header, sidebar, toolbar), and **View** (contenido: tablas, KPIs, gráficos). Documented in `prompts/FEATURE-AUDIT.md`. Examples:
  ...
  - Organization Finances: `OrganizationFinances.tsx` (Page) → `LabLayout`/`DashboardLayout` (Layout) → `OrganizationFinancesView` (View)
  - Project Finances: `ProjectFinances.tsx` (Page) → `LabLayout`/`DashboardLayout` (Layout) → `ProjectFinancesView` (View)
  - Contacts: `Contacts.tsx` (Page) → `LabLayout`/`DashboardLayout` (Layout) → `ContactsView` (View)
...
- **Lab Layout Drawer System**: Right-side drawer component (`LabDrawer`) that appears below the header for displaying forms and page-specific content. Managed via `useLabDrawerStore` with `openDrawer({ title, subtitle, content, width })` and `closeDrawer()`. Width variants: sm (320px), md (400px), lg (480px), xl (560px). Can also be controlled via props (`isDrawerOpen`, `onDrawerClose`, `drawerContent`).
...
- **Popover Positioning System (Sidebar)**: Avatar, Help, and Notification popovers use `Popover` (not `DropdownMenu`) positioned with `side="right"` and `align="end"` to appear to the right of buttons with aligned bottom edges. Fixed layout interference issue where `DropdownMenu` was creating unwanted spacing affecting scrollbar.
