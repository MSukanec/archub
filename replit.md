...
- **LISTA Tab Performance Optimization**: Created simplified `client_list_view` using ONLY base tables (project_clients, contacts, users, client_roles) with ONLY fields displayed in LISTA tab. NO dependency on other views. NO financial data (not used in LISTA). Both project and organization handlers now use single SELECT from view with simple mapping to client objects.
