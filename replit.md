...
- **Client Modal Refactoring (Nov 25, 2024)**: Refactored `ProjectClientModal` from `src/features/clients/modals/ClientDataModal.tsx` to `src/features/clients/forms/ClientForm.tsx` following Modal-Standard.md. Implemented FormPanel (create/edit) and ViewPanel (view) subcomponents. Migrated to ModalLayout/ModalHeader/ModalBody/ModalFooter API from deprecated FormModalLayout. Registry updated with `mapDataToProps` for automatic mode detection (mode='create' by default, 'edit' when clientId present, 'view' when explicitly set). Size reduced to 'md' for better UX. Supports CREATE/EDIT/VIEW modes in single component with conditional rendering based on mode prop. **Payment Modal Refactoring (Nov 25, 2024)**: Refactored `ClientPaymentsModal` from `src/features/clients/modals/ClientPaymentsModal.tsx` to `src/features/clients/forms/ClientPaymentForm.tsx` following Modal-Standard.md. Implemented FormPanel (create/edit) with complex form layout (date, client, wallet, amount, currency, exchange rate, status, reference, notes, file upload) and ViewPanel (view) with detailed payment information display. Migrated to ModalLayout/ModalHeader/ModalBody/ModalFooter API. Registry updated with `mapDataToProps` for both 'client-payment' and 'installment' modals. Size set to 'lg' to accommodate extensive form fields. Single component handles CREATE/EDIT/VIEW modes with proper loading states and error handling.
