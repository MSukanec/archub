...
- **Chart Library Architecture**: Type-based folder structure (`line/`, `bar/`, `pie/`, `radial/`, `composed/`, `heatmap/`, `sparkline/`, `table/`) with unified theme system (`theme.ts`), visual catalog (`CATALOG.md`), and centralized exports (`index.ts`). All charts follow Nivel 1 pattern: pure visualization, no Card wrappers, no business logic. Legacy charts in `legacy/`, `gantt/`, `courses/` folders pending migration.
