...
- **Client Modal Refactoring (Nov 25, 2024)**: Refactored three client modals to follow Modal-Standard.md: (1) `ProjectClientModal` → `ClientForm.tsx` - handles CREATE/EDIT/VIEW of clients with FormPanel and ViewPanel subcomponents. Size 'md'. (2) `ClientPaymentsModal` → `ClientPaymentForm.tsx` - handles payments with complex form (date, client, wallet, amount, currency, exchange rate, status, reference, notes, attachments). Size 'lg'. (3) `ClientRoleModal` → `ClientRoleForm.tsx` - handles custom client role creation/editing. Size 'sm'. All migrated from deprecated FormModalLayout to ModalLayout/ModalHeader/ModalBody/ModalFooter API. Registry updated with `mapDataToProps` for automatic mode detection. All three modals removed from `modals/` folder and consolidated into `forms/` folder.
