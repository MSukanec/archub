...
- **Universal Import System (Nov 26, 2024)**: Created universal import component `UniversalImportForm` at `src/components/forms/imports/`. Architecture: (1) 5-step wizard (Preview → Mapping → Validation → Conflicts → Summary). (2) Reusable hooks: `useFileParser` (CSV/Excel parsing with xlsx+papaparse), `useColumnAutoMap` (smart column mapping with Levenshtein distance), `useValueNormalizer` (fuzzy value matching), `useValidationEngine` (field validation with foreign key support). (3) Entity-agnostic API via `ImportConfig`: `targetSchema` defines fields, `valueMapConfig` provides value mappings, `onImport` callback receives processed rows. (4) Modal-Standard.md compliant using ModalLayout/Header/Body/Footer. (5) Supports 10-10,000 rows import. (6) Legacy modal `MovementImportStepModal.tsx` (1889 lines) preserved for reference. (7) Registered as `universal-import` in registerModals.ts. Usage: `openModal('universal-import', { config: { entityName, targetSchema, valueMapConfig, onImport } })`.
