...
- **Movement Third Party Contributions System (2025-08-08)**: Refactored installment/contribution system to use new `movement_third_party_contributions` junction table instead of direct `contact_id` in movements table. This architectural change better separates concerns - movements table now focuses on financial data while third-party relationships are managed separately. Updated InstallmentFormModal to create/update both movement and contribution records. Added support for receipt numbers. Schema updated in shared/schema.ts with new table definition and corresponding TypeScript types.
