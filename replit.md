...
- **Onboarding Flow Fix**: Corrected critical issue where new users could bypass onboarding process. Fixed OAuth redirect to use root path instead of direct dashboard redirect, eliminated protected routes during onboarding allowlist, and implemented automatic user data initialization with onboarding_completed=false for new users. Now enforces proper onboarding flow for all new registrations (September 2025).
