...
- **UploadMediaField Component Refactor**: Migrated `UploadGallery.tsx` to `UploadMediaField.tsx` in `src/components/ui-custom/fields/` with UX redesign based on reference pattern. Key changes: (1) Upload button now integrated into empty state instead of separate top button, (2) Files displayed in vertical list with card format (thumbnail left, name/size/progress, delete button right) instead of grid layout, (3) Added upload progress bars with percentage display, (4) Maintained lightbox functionality for image preview. Updated all references in MediaForm.tsx (sitelog) and ContactAttachmentsForm.tsx (contacts).
- **FormModalLayout Auto-Back Button**: Implemented automatic back button injection for all subforms at FormModalLayout level using React.cloneElement. When `effectivePanel === 'subform'`, the layout automatically adds `showBackButton={true}` and `onBackClick={() => setPanel('edit')}` to headerContent. This ensures consistent back navigation across all modals (SiteLogModal, ContactModal, TaskMultiModal) without manual implementation in each modal. Simplified SiteLogModal to remove redundant manual back button code.
- **PAGOS Tab KPI Metrics System**: Implemented complete payment metrics system with 4 KPI cards showing Pagos (total count), Total Confirmado (confirmed payments sum), Total Pendiente (pending payments sum), and Último Pago (latest payment date). Created `paymentsMetrics.ts` service with currency-based aggregation and NULL-currency fallback (UNKNOWN bucket). Added endpoints `/api/projects/:projectId/client-payments/metrics` and `/api/organizations/:organizationId/client-payments/metrics` with automatic cache invalidation on mutations. Fixed critical bugs: (1) NULL currency_id payments now aggregate to UNKNOWN instead of being dropped, (2) Frontend query URL now includes `?organization_id=` parameter. KPIs refresh instantly after payment create/edit/delete operations.
