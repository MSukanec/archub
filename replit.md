...
- **Universal Import System v2.0 (Nov 26, 2024)**: Refactored to Feature-Sliced Design module at `src/features/imports/`. Architecture: (1) 5-step wizard (Preview → Mapping → Validation → Conflicts → Summary). (2) Reusable hooks: `useFileParser` (CSV/Excel parsing with xlsx+papaparse), `useColumnAutoMap` (smart column mapping with Levenshtein distance), `useValidationEngine` (field validation with foreign key support), `useAISuggestMapping` (AI-powered suggestions with memory). (3) Entity-agnostic API via `ImportConfig`: `targetSchema` defines fields, `valueMapConfig` provides value mappings, `onImport` callback receives processed rows. (4) Modal-Standard.md compliant using ModalLayout/Header/Body/Footer. (5) Supports 10-10,000 rows import. (6) **AI-Powered Column Mapping**: Uses OpenAI gpt-4o-mini + organizational memory (`ia_import_mapping_patterns` table). Memory lookups return 100% confidence (green badge), AI suggestions show 70-100% confidence (blue badge). Patterns auto-save when user confirms mapping (step 2→3). Headers normalized to lowercase/trim before storage. (7) Backend endpoints: `POST /api/import/ai-suggest-mapping`, `POST /api/import/save-mappings` in `server/controllers/import/importAI.controller.ts`. (8) StepMapping shows AI confidence badges and loading indicator during AI analysis. (9) Legacy modal `MovementImportStepModal.tsx` (1889 lines) preserved for reference. Usage: `openModal('universal-import', { config: { entityName, targetSchema, valueMapConfig, onImport } })`.
- **Client Unit Migration (Nov 26, 2024)**: Major schema migration removing `unit` from `project_clients` and adding `unit_name` + `unit_description` to `client_commitments`. Architecture change: One client per project regardless of units; unit information is now commitment-specific, not client-specific. (1) Removed `unit` field from: PROJECT_CLIENTS table, ProjectClient type, ClientListItem, ProjectClientSummary interface, all mappers and services. (2) Added `unit_name` (text) and `unit_description` (text) to CLIENT_COMMITMENTS table and ClientCommitment type. (3) ClientForm.tsx: Now only has Contacto and Rol fields (unit removed). (4) ClientCommitmentForm.tsx: Added unit_name input field (below client selector) and unit_description textarea (below currency fields). (5) Deprecated and removed `ClientObligationModal.tsx` - functionality fully replaced by ClientCommitmentForm.tsx. (6) Updated ClientObligationRow to show `unit_name` from commitments. (7) Cleaned all `project_clients.unit` references from ClientPaymentRow, ClientPaymentForm, Client.tsx, and related services. SQL migration required: DROP unit from project_clients, ADD unit_name/unit_description to client_commitments, migrate existing unit data to commitments.
