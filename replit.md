...
- **Proration System**: Fair upgrade pricing via `calculateProration()` in `server/lib/handlers/checkout/shared/proration.ts`. Calculates remaining value from current subscription as credit, applied to new plan price. Supports both USD (PayPal) and ARS (MercadoPago). Frontend displays breakdown in checkout. Backend recalculates server-side for security (never trusts client-provided amounts). Endpoint: `POST /api/checkout/calculate-proration`.
