...
- **TASKS Module**: Consolidated feature in `src/features/tasks/` with barrel exports. Contains 18 hooks, 8 modals, 15 components, 3 views (TaskBasicDataView, TaskCostsView, TasksView). Accessible via project sidebar at `/project/tasks` (currently restricted as `coming_soon`). Pattern reference: GENERAL-COSTS feature.
...
- **Catalog Folder Structure**: Renamed from `analysis` to `catalog` at `src/pages/professional/catalog/` containing task and material-related views. TaskDetailPage moved to `src/pages/dashboard/TaskDetailPage.tsx` for detail views with multiple tabs.
