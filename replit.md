...
- **Learning Module Implementation**: Added new "Capacitaciones" (Learning) module with dedicated sidebar context and routing. SelectMode.tsx redesigned from 2x2 grid to single row of 5 mode options: Profesional, Capacitaciones, Proveedores, Contratistas, Visitantes. Created learning section in src/pages/learning with LearningDashboard.tsx and CourseList.tsx placeholder pages. Added 'learning' to SidebarContext and SidebarLevel types in navigationStore.ts. Updated Sidebar.tsx with learning navigation items (Dashboard, Cursos). Added 'learner' to user_type enum in shared/schema.ts to match database enum (October 2025).
- **Budget API Fix**: Fixed critical bug in /api/budgets endpoint where query attempted to select non-existent columns (construction_task_id, labor_unit_cost, material_unit_cost) from budget_items table. Simplified endpoint to calculate only total budget values using existing columns (unit_price, quantity, markup_pct, tax_pct), eliminating database errors and improving API reliability. Removed obsolete "Materiales" and "Mano de Obra" columns from BudgetItemTab.tsx since budget_items table no longer stores separate labor/material cost breakdowns (October 2025).
