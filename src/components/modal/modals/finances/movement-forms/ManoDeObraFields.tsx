...
export function ManoDeObraFields({ form, currencies, wallets, members, concepts, selectedTaskIds, setSelectedTaskIds, onOpenTasksSubform }: Props) {
  ...
  return (
    <>
      {/* Separador y título de sección de mano de obra */}
      ...
      <div className="col-span-2">
        <FormSubsectionButton
          ...
          description={selectedTaskIds.length > 0 ? `${selectedTaskIds.length} tarea${selectedTaskIds.length === 1 ? '' : 's'} seleccionada${selectedTaskIds.length === 1 ? '' : 's'}` : "Selecciona las tareas relacionadas con este pago"}
