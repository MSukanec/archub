  ...
  return (
    ...
    >
      <div className="flex items-center gap-4">
            ...
            {contact_types && contact_types.length > 0 && !linked_user && (
              <div className="flex gap-1 mt-1 flex-wrap">
                {contact_types.slice(0, 2).map((typeLink: any, index: number) => (
                  <Badge key={`${typeLink.type_id}-${index}`} variant="secondary" className="text-xs px-1 py-0">
        ...
        <div className="flex-1 min-w-0">
          {email && (
            <div className="text-sm text-muted-foreground truncate" title={email}>
              {email}
            </div>
          )}
          {phone && (
            <div className="text-sm text-muted-foreground truncate mt-0.5" title={phone}>
              {phone}
            </div>
          )}
          {!email && !phone && (
