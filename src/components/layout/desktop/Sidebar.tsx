...
export function Sidebar() {
  ...
  return (
    <aside
      ...
      className={cn(
        "fixed left-0 top-12 z-30 bg-slate-50/95 dark:bg-slate-900/95 backdrop-blur-sm border-r-2 border-white dark:border-slate-800 shadow-sm transition-all duration-300 ease-in-out",
        "h-[calc(100vh-3rem)]",
        isExpanded ? "w-64" : "w-12",
        isExpanded && !isDocked && "shadow-2xl"
      )}
      style={{
        overflow: 'hidden'
      }}
