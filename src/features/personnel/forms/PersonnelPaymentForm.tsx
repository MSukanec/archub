...
export function FormPanel({
  ...
  const personnelOptions = useMemo(() => {
    ...
    return personnel
      .map(p => {
        const name = formatContactName(p.contact)
        return {
          value: p.id,
          label: name
        }
            ...
            <FormItem>
              ...
              <FormControl>
                <ComboBox
                  ...
                  data-testid="combobox-personnel-payment-personnel"
                />
              </FormControl>
              <FormMessage />
            </FormItem>
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {existingPayment.personnel?.contact && (
          <div>
            <h4 className="text-xs font-medium text-muted-foreground mb-1.5">Personal</h4>
            <div className="flex flex-col">
              <span className="text-base font-semibold" data-testid="text-personnel-payment-person-name">
                {formatContactName(existingPayment.personnel.contact) || '-'}
              </span>
            </div>
          </div>
        )}
  ...
  useEffect(() => {
    if (existingPayment && (mode === 'edit'|| mode === 'view')) {
      ...
      form.reset({
        ...
        personnel_id: existingPayment.personnel_id || undefined,
      ...
      if (filesToUpload.length > 0 && createdPaymentId) {
        ...
        for (const fileInput of filesToUpload) {
          try {
            ...
            if (!fileInput.file) {
              console.error('[PersonnelPaymentFormFields] No file object in fileInput:', fileInput)
              continue
            }
            ...
            console.log('[PersonnelPaymentFormFields] Upload successful:', uploadResult)
          } catch (uploadError: any) {
            console.error('[PersonnelPaymentFormFields] Error uploading file:', {
              error: uploadError,
