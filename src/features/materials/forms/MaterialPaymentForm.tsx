...
export function MaterialPaymentForm({ modalData, onClose, mode = 'create' }: MaterialPaymentFormProps) {
    ...
    try {
      ...
      if (filesToUpload.length > 0 && createdPaymentId) {
        ...
        for (const fileInput of filesToUpload) {
          try {
            console.log('[MaterialPaymentForm] Uploading file:', {
            ...
            if (!fileInput.file) {
              console.error('[MaterialPaymentForm] No file object in fileInput:', fileInput)
              continue
            }
            ...
            console.log('[MaterialPaymentForm] Upload successful:', uploadResult)
          } catch (uploadError: any) {
            console.error('[MaterialPaymentForm] Error uploading file:', {
              error: uploadError,
    ...
    <ModalLayout onClose={handleClose} size="lg">
      ...
      {mode === 'view' && (
        <ModalFooter
          leftLabel="Cerrar"
          onLeftClick={handleClose}
          data-testid="footer-material-payment-view"
        />
      )}
    </ModalLayout>
  )
...
export default MaterialPaymentForm
