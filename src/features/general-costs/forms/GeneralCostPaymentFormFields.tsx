...
export function GeneralCostPaymentFormFields({
  ...
  const { data: currencies, isLoading: currenciesLoading } = useOrganizationCurrencies(effectiveOrganizationId)
  const { data: generalCosts, isLoading: generalCostsLoading } = useGeneralCosts(effectiveOrganizationId || null)
  ...
  const onSubmit = async (data: GeneralCostPaymentFormData) => {
    if (!effectiveOrganizationId || !userData?.user?.id) {
