 ...
 * 
 * Rules:
 * - Two words: first letter of each word ("Casa Verde" → "CV")
 * - One word: first two letters ("Mascardi" → "MA", "Villarino" → "VI")
 * - Empty/whitespace: "PR" (PRoject)
 * - Normalize: trim, uppercase, collapse whitespace
 * - Handle hyphenated names properly
 * 
...
export function getProjectInitials(name: string): string {
  ...
  if (words.length === 1) {
    ...
    // Handle hyphenated names like "Casa-Verde" - treat as two words
    if (word.includes('-')) {
      const hyphenatedParts = word.split('-').filter(part => part.length > 0);
      if (hyphenatedParts.length >= 2) {
        return (hyphenatedParts[0][0] + hyphenatedParts[1][0]).toUpperCase();
