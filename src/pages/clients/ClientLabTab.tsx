...
function HeatmapView({ clients, onClientClick }: HeatmapViewProps) {
  ...
  return (
    <div className="absolute inset-0 overflow-auto p-6 pt-20">
...
interface ClientLabTabProps {
  projectId?: string;
}

export default function ClientLabTab({ projectId }: ClientLabTabProps) {
  ...
  const { data: projectData } = useQuery({
    queryKey: ['project-lite', activeProjectId],
    queryFn: async () => {
  ...
  const { data: clientsSummary, isLoading: clientsLoading } = useQuery({
    queryKey: ['clients-summary-lab', activeProjectId, organizationId],
    queryFn: () => fetchClientsSummary(activeProjectId!, organizationId!),
  ...
  const hasNoClients = !isLoading && clientNodes.length === 0 && activeProjectId;
  const hasNoProject = !activeProjectId;

  return (
    <div 
      ref={containerRef}
      className="absolute inset-0 bg-[#0a0a0f] overflow-hidden"
    ...
    >
      ...
      <div className="absolute top-4 left-4 z-10 flex flex-col gap-2">
        <div className="bg-black/60 backdrop-blur-md rounded-lg border border-white/10 p-4">
          <h2 className="text-lg font-bold text-white mb-1">Financial Neural Lattice</h2>
          <p className="text-xs text-white/50">Visualización orgánica del estado financiero</p>
        </div>
      ...
      {hasNoProject && (
        <div className="absolute inset-0 flex items-center justify-center z-20">
          <div className="flex flex-col items-center gap-4 text-center">
            ...
            <p className="text-white/70 text-lg">Selecciona un proyecto</p>
            <p className="text-white/40 text-sm">Usa el selector de proyecto en el header</p>
          </div>
        </div>
      )}
        ...
        {selectedNode && (
          <>
            ...
            <motion.div
              ...
              className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-50 w-[400px] max-w-[90vw]"
            >
              <div 
