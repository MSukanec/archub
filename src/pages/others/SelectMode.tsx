...
export default function SelectMode() {
  ...
  const handleModeSelect = (modeType: ModeOption['type']) => {
    ...
    if (hasFinished || updateUserTypeMutation.isPending) {
      console.log('SelectMode - Preventing duplicate click', { hasFinished, isPending: updateUserTypeMutation.isPending });
      return;
    }
  ...
  const headerProps = {
    title: "Elegir modo de uso",
    subtitle: "Cambia cómo usas Archub para personalizar tu experiencia",
    icon: CheckCircle,
    breadcrumbs: [
      { label: "Otros", href: "#" },
      { label: "Modo de uso", href: "/select-mode" }
    ]
  };
                ...
                const cardContent = (
                  <Card
                    ...
                    className={`
                      ...
                      ${updateUserTypeMutation.isPending ? 'opacity-75' : ''}
                    `}
                    onClick={() => isAvailable && !updateUserTypeMutation.isPending && !hasFinished && handleModeSelect(mode.type)}
                  >
                    <CardHeader className="pb-2">
                      <div className="flex items-center justify-between">
                        ...
                        {(updateUserTypeMutation.isPending || hasFinished) && isSelected && (
